#!/usr/bin/env python3
"""
Script for generating data in the required format for fine-tuning OpenAI models.
Takes data from JSON schema definition and generates JSONL files for training, validation, and testing
in the OpenAI/ChatGPT format.
"""
...
# Define a type for OpenAI format message
MessageType = dict[str, str]
# Define a type for OpenAI format example
ExampleType = dict[str, list[MessageType]]
...
def create_fine_tuning_example(
    ...
    """Create a single fine-tuning example in the OpenAI format."""
    system_content = "You are an AI assistant that translates natural language queries into SQL. Generate only the SQL query."
    user_content = (
        f"Translate the following natural language query to SQL. Question: {question}"
    )
...
def save_jsonl(items: list[ExampleType], output_file: str) -> None:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Generate data for fine-tuning OpenAI models"
    )
    parser.add_argument(
        "--eval-dataset",
        required=True,
    ...
    save_jsonl(
        val_set, str(output_dir / "valid.jsonl")
    )  # OpenAI uses "validation" instead of "valid"
    save_jsonl(test_set, str(output_dir / "test.jsonl"))

    print("Generated datasets in OpenAI format:")
